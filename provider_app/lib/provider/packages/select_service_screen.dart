...
class SelectServiceScreen extends StatefulWidget {
  ...
  SelectServiceScreen({this.categoryId, this.subCategoryId, this.isUpdate = false, this.packageData, this.languageCode});
...
class _SelectServiceScreenState extends State<SelectServiceScreen> {
  ...
  Future<void> fetchAllServices({String? searchText = "", int? categoryId = -1, int? subCategoryId = -1}) async {
    ...
    await getServicesList(page, search: searchText, categoryId: categoryId, subCategoryId: subCategoryId, providerId: appStore.userId, type: SERVICE_TYPE_FIXED, languageCode: widget.languageCode.validate()).then((value) {
      isApiCalled = true;
  ...
  Future<void> getCategory() async {
    ...
    await getCategoryList(perPage: CATEGORY_TYPE_ALL, languageCode: widget.languageCode.validate()).then((value) async {
  ...
  Future<void> getSubCategory({required int categoryId}) async {
    getSubCategoryList(catId: categoryId.toInt(), languageCode: widget.languageCode.validate()).then((value) async {
