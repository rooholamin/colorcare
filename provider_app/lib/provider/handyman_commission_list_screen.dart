...
class HandymanCommissionTypeListScreen extends StatefulWidget {
  ...
  @override
  _HandymanCommissionTypeListScreenState createState() => _HandymanCommissionTypeListScreenState();
}

class _HandymanCommissionTypeListScreenState extends State<HandymanCommissionTypeListScreen> {
  ...
  Future<void> restoreProviderHandymanTypeList({required int commissionId}) async {
    appStore.setLoading(true);
    var req = {
  ...
  Future<void> forceDeleteProviderHandymanTypeList({required int commissionId}) async {
    appStore.setLoading(true);
    var req = {
      CommonKeys.id: commissionId,
      "type": FORCE_DELETE,
  ...
  void init() async {
    future = getCommissionType(type: USER_TYPE_HANDYMAN, providerId: appStore.userId, isDelete: true);
  }
    ...
    return AppScaffold(
      ...
      actions: [
        IconButton(
          ...
          onPressed: () async {
            bool? res = await AddHandymanCommissionTypeListScreen().launch(context);
            if (res ?? false) {
              page = 1;
              init();
              setState(() {});
              ...
              return Container(
                ...
                child: Column(
                  children: [
                    Row(
                      children: [
                        ...
                        Column(
                          ...
                          children: [
                            Text(languages.handymanCommission, style: secondaryTextStyle(size: 12)),
                            4.height,
                            Text(
                        ...
                        if (isProvider && (rolesAndPermissionStore.handymanTypeEdit || rolesAndPermissionStore.handymanTypeDelete))
                          PopupMenuButton(
                            icon: const Icon(Icons.more_vert, size: 24, color: appTextSecondaryColor),
                            color: context.cardColor,
                            onSelected: (selection) async {
                              if (selection == 1) {
                                bool? res = await AddHandymanCommissionTypeListScreen(typeData: typeData).launch(context);
                                if (res ?? false) {
                                  page = 1;
                                  init();
                                  setState(() {});
                              ...
                              if (selection == 2) {
                                showConfirmDialogCustom(
                                  ...
                                  onAccept: (_) {
                                    ifNotTester(context, () {
                                      removeProviderHandymanTypeList(typeData.id.validate());
                                    });
                                  },
                                );
                              } else if (selection == 3) {
                                showConfirmDialogCustom(
                                  ...
                                  onAccept: (_) {
                                    ifNotTester(context, () {
                                      restoreProviderHandymanTypeList(commissionId: typeData.id.validate());
                                    });
                                  },
                                );
                              } else if (selection == 4) {
                                showConfirmDialogCustom(
                                  ...
                                  onAccept: (_) {
                                    ifNotTester(context, () {
                                      forceDeleteProviderHandymanTypeList(commissionId: typeData.id.validate());
                                    });
                                  },
                                );
                              }
                            ...
                            itemBuilder: (context) => [
                              if (rolesAndPermissionStore.handymanTypeEdit)
                                PopupMenuItem(
                                  child: Text(languages.lblEdit, style: boldTextStyle(color: typeData.deletedAt == null ? textPrimaryColorGlobal : textSecondaryColor)),
                                  value: 1,
                                  enabled: typeData.deletedAt == null,
                                ),
                              if (rolesAndPermissionStore.handymanTypeDelete)
                                PopupMenuItem(
                                  child: Text(languages.lblDelete, style: boldTextStyle(color: typeData.deletedAt == null ? textPrimaryColorGlobal : textSecondaryColor)),
                                  value: 2,
                                  enabled: typeData.deletedAt == null,
                                ),
                              PopupMenuItem(
                                child: Text(languages.lblRestore, style: boldTextStyle(color: typeData.deletedAt != null ? textPrimaryColorGlobal : textSecondaryColor)),
                                value: 3,
                                enabled: typeData.deletedAt != null,
                              ),
                              if (rolesAndPermissionStore.handymanTypeDelete)
                                PopupMenuItem(
                                  child: Text(languages.lblForceDelete, style: boldTextStyle(color: typeData.deletedAt != null ? textPrimaryColorGlobal : textSecondaryColor)),
                                  value: 4,
                                  enabled: typeData.deletedAt != null,
                                ),
                            ],
                    ...
                    Row(
                      children: [
                        Image.asset(percent_line, height: 18, color: context.primaryColor),
                        16.width,
                        Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(languages.commission, style: secondaryTextStyle(size: 12)),
                            4.height,
                            if (typeData.type.validate().toLowerCase() == COMMISSION_TYPE_PERCENT.toLowerCase())
                              Text(
                                '${typeData.commission}%',
                                style: boldTextStyle(size: 12),
                              ),
                            if (typeData.type.validate().toLowerCase() == COMMISSION_TYPE_FIXED.toLowerCase()) Text(typeData.commission.validate().toPriceFormat(), style: boldTextStyle(size: 12)),
                          ],
                        ),
                      ],
                    ),
                        ...
                        Row(
                          children: [
                            ...
                            Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(languages.lblStatus, style: secondaryTextStyle(size: 12)),
                                4.height,
                                Text(
                                  typeData.status == 1 ? ACTIVE.toUpperCase() : INACTIVE.toUpperCase(),
                                  style: boldTextStyle(size: 12, color: typeData.status == 1 ? greenColor : Colors.redAccent),
                                ),
                              ],
                            ),
                          ],
