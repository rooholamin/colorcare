  ...
  Map<String, dynamic> _buildServiceRequest() {
    ...
    if (selectedVisitType!.key == VISIT_OPTION_SHOP) {
      if (selectedShops.isEmpty) {
        ...
        final shopIds = selectedShops.where((element) => element.isSelected).map((e) => e.id.validate()).toList();
          ...
          children: [
            ...
            if (selectedVisitType?.key == VISIT_OPTION_SHOP)
              ServiceShopComponent(
                selectedList: widget.data?.shop.map((shop) => shop.id.validate()).toList(),
                onSelectedList: (val) {
                            ...
                            showConfirmDialogCustom(
                              ...
                              onAccept: (p0) {
                                ...
                                if (value.startsWith('http')) {
                                  removeAttachment(
                                    id: tempAttachments.validate().firstWhere((element) => element.url == value).id.validate(),
                                  );
                                }
                              },
                            );
