...
void forceUpdate(
  ...
  showInDialog(
    ...
    barrierDismissible: currentAppVersionCode >= getIntAsync(PROVIDER_APP_MINIMUM_VERSION),
    builder: (_) {
      return PopScope(
        canPop: false,
        child: NewUpdateDialog(
          canClose: getBoolAsync(UPDATE_NOTIFY) ? currentAppVersionCode >= getIntAsync(PROVIDER_APP_MINIMUM_VERSION) : true,
        ),
      );
    },
  );
...
Future<void> showForceUpdateDialog(BuildContext context) async {
  if (getBoolAsync(UPDATE_NOTIFY, defaultValue: true)) {
    getPackageInfo().then((value) {
      if ((isAndroid || isIOS) && getIntAsync(PROVIDER_APP_LATEST_VERSION) > value.versionCode.validate().toInt()) {
        forceUpdate(
          context,
              ...
              children: List.generate(
                ...
                (index) {
                  ...
                  return ElevatedButton(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Text(
                          ...
                          style: secondaryTextStyle(
                            color: appStore.selectedLanguage.languageCode == languageData.languageCode ? white : textSecondaryColorGlobal,
                          ),
                        ),
                      ],
                    ),
                    ...
                    right: 8,
                    left: languageList().first.languageCode == languageData.languageCode ? 16 : 0,
                  );
                },
              ),
            ),
