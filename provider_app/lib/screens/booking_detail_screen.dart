...
class BookingDetailScreenState extends State<BookingDetailScreen> with WidgetsBindingObserver {
              ...
              GestureDetector(
                onTap: () {
                  if (bookingResponse.bookingDetail!.isPostJob || bookingResponse.bookingDetail!.isPackageBooking) {
                    ...
                    ServiceDetailScreen(
                      serviceId: bookingResponse.bookingDetail!.serviceId.validate(),
                    ).launch(context);
                  }
                },
                child: Row(
  ...
  Widget _action({required BookingDetailResponse res}) {
    final isShopBooking = (res.bookingDetail?.bookingType == VISIT_OPTION_SHOP) || (res.bookingDetail?.visitType == VISIT_OPTION_SHOP) || (res.service?.isOnShop == true);
  ...
  Widget handleProvider({required BookingDetailResponse res, required bool isShopBooking}) {
    if (isShopBooking) {
      ...
      } else if (res.bookingDetail!.status == BookingStatusKeys.complete && res.bookingDetail!.paymentMethod == PAYMENT_METHOD_COD && res.bookingDetail!.paymentStatus == PENDING) {
        showBottomActionBar = true;
        return AppButton(
      ...
      } else if (res.bookingDetail!.status == BookingStatusKeys.complete && (res.bookingDetail!.paymentStatus == PAID || res.bookingDetail!.paymentStatus == PENDING_BY_ADMINS)) {
        showBottomActionBar = true;
        return AppButton(
  ...
  Widget handleHandyman({required BookingDetailResponse res, required bool isShopBooking}) {
    if (isShopBooking) {
      ...
      if (res.bookingDetail!.status == BookingStatusKeys.accept && res.bookingDetail?.visitType != VISIT_OPTION_SHOP) {
        showBottomActionBar = true;
        return Container(
          child: Row(
            children: [
        ...
        Row(
          ...
          children: [
            ...
            IconButton(
              ...
              res.bookingDetail!.paymentStatus != PAID && res.bookingDetail!.paymentStatus != PENDING_BY_ADMINS,
            ),
          ],
        ),
        16.height,
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      4.width,
                      PriceWidget(
                        price: '${data.price.validate() * data.qty.validate()}'.toDouble(),
                        size: 16,
            ...
            children: [
              AnimatedScrollView(
                ...
                children: [
                  ...
                  if (res.data!.isBookedAtShop) shopDetailWidget(shop: res.data!.shop!),

                  /// Total Service Time
                  _buildCounterWidget(value: res.data!),

                  /// Location Tracking
                  locationTrackWidget(data: res.data).visible(BookingStatusKeys.onGoing == res.data!.bookingDetail!.status &&
                      !isUserTypeHandyman &&
                      res.data!.handymanData![0].id != appStore.userId &&
                      // Hide map for shop bookings
                      !((res.data!.bookingDetail?.bookingType == VISIT_OPTION_SHOP) || (res.data!.bookingDetail?.visitType == VISIT_OPTION_SHOP) || (res.data!.service?.isOnShop == true))),

                  /// My Service List
                  if (res.data!.postRequestDetail != null && res.data!.postRequestDetail!.service != null)
                    myServiceList(
                          ...
                          Column(
                            ...
                            children: [
                              ...
                              Text.rich(
                                TextSpan(
                                  children: [
                                    ...
                                    TextSpan(
                                      text: " ${DateTime.parse(handymanLocation?.data.datetime.toString() ?? DateTime.now().toString()).timeAgo}",
                                      style: secondaryTextStyle(size: 12),
                                    ),
                                  ],
                                ),
                              ...
                              TextButton(
                                ...
                                onPressed: () {
                                  startLocationUpdates(
                                    status: res.data?.bookingDetail?.status.validate() ?? "",
                                    handymanID: res.data?.handymanData?.first.id.validate() ?? -1,
                                  );
                                },
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  ...
                                  Row(
                                    children: [
                                      Text(res.data!.bookingDetail!.txnId.validate(), textAlign: TextAlign.right, style: boldTextStyle(color: pending), maxLines: 1, overflow: TextOverflow.ellipsis)
                                          .expand(),
                                      4.width,
                                      InkWell(
                                        onTap: () async {
              ...
              Positioned(
                ...
                child: Container(
                  ...
                  padding: showBottomActionBar ? const EdgeInsets.all(16) : EdgeInsets.zero,
                ),
              ),
            ],
          ),
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  ImageBorder(
                    src: shop.shopFirstImage.isNotEmpty ? shop.shopFirstImage : Assets.iconsIcDefaultShop,
                    height: 60,
                  ),
                  16.width,
                  Column(
                    ...
                    children: [
                      ...
                      if (shop.shopStartTime.isNotEmpty && shop.shopEndTime.isNotEmpty) ...[
                        TextIcon(
                          spacing: 10,
                          prefix: Image.asset(ic_time_slots, width: 12, height: 12, color: context.iconColor),
