...
class BookingController extends Controller
{
    ...
    public function initiate(Request $request)
    {
        ...
        try {
            ...
            // Use proper V1 keys from configuration
            $merchantId = $phonepaymentSettings['merchant_id_v1'] ?? $phonepaymentSettings['merchant_id'] ?? '';
            $saltKey = $phonepaymentSettings['salt_key_v1'] ?? $phonepaymentSettings['salt_key'] ?? '';
            $saltIndex = $phonepaymentSettings['salt_index_v1'] ?? $phonepaymentSettings['salt_index'] ?? 1;

            if(empty($merchantId) || empty($saltKey)) {
                return response()->json(['error' => 'PhonePe V1 payment gateway is not configured properly.'], 400);
            }
            ...
            $checksum = $this->generateChecksum($base64Payload, $apiPath, $saltKey, $saltIndex);
    ...
    public function callback(Request $request)
    {
        ...
        $decoded = $response;
        $phonePeTransactionId = $decoded['transactionId'] ?? null;

        // Example structure: check transaction status
        if (isset($decoded['code']) && $decoded['code'] === 'PAYMENT_SUCCESS') {
            return redirect('/payment/success?transaction_id=' . $phonePeTransactionId)->with('success','Payment successful');
        } else {
            // Handle failure or pending
            return redirect('/payment/failed?transaction_id=' . $phonePeTransactionId)->with('error','Payment failed');
        }
     ...
     */
    protected function getAccessTokenV2($phonepaymentSettings)
    {
        try {
            // Use asForm() to properly encode form data
            $response = Http::asForm()->post('https://api-preprod.phonepe.com/apis/pg-sandbox/v1/oauth/token', [
                'client_id' =>  $phonepaymentSettings['client_id_v2'],
                'client_version' => 2,
                'client_secret' => $phonepaymentSettings['client_secret_v2'],
                'grant_type' => 'client_credentials'
            ]);

            // For debugging - remove this in production
            if (!$response->successful()) {
                \Log::error('PhonePe V2 Token Error:', [
            ...
            \Log::error('PhonePe V2 Token Exception: ' . $e->getMessage());
            return null;
        }
    }
    ...
    public function initiateV2(Request $request)
    {
        ...
        try {
            ...
            // Check if v2 configuration is properly set
            if(empty($phonepaymentSettings['phonepe_version']) || $phonepaymentSettings['phonepe_version'] !== 'v2') {
                return response()->json(['error' => 'PhonePe is not configured for V2. Please check gateway settings.'], 400);
            }

            // Check required V2 settings
            if (empty($phonepaymentSettings['merchant_id_v2']) ||
                empty($phonepaymentSettings['client_id_v2']) ||
                empty($phonepaymentSettings['client_secret_v2'])) {
                return response()->json(['error' => 'PhonePe V2 payment gateway is not configured properly.'], 400);
            }

            // Get access token
            $accessToken = $this->getAccessTokenV2($phonepaymentSettings);
      
            if (!$accessToken) {
                return response()->json(['error' => 'Failed to get access token from PhonePe.'], 400);
            }

            $amount = (float) $request->amount;

            // Generate unique merchant order ID
            $merchantOrderId = 'TXN_' . '_' . time();

            $payload = [
                "merchantOrderId" => $merchantOrderId,
                "amount" => intval(round($amount * 100)), // Amount in paisa as integer
                "expireAfter" => 1200, // 20 minutes expiry
            
                "paymentFlow" => [
                    "type" => "PG_CHECKOUT",
                    "message" => "Payment for #" . $merchantOrderId,
                    "merchantUrls" => [
                        "redirectUrl" => url('/api/phonepe/callback-v2?merchantOrderId=' . $merchantOrderId)
                    ]
                ]
            ];
    ...
    /**
     * Verify PhonePe V2 payment status
     */
    protected function verifyPaymentStatusV2($merchantOrderId, $accessToken)
    {
        \Log::info('Verifying payment status for merchantOrderId: ' . $merchantOrderId);
        try {
            ...
            \Log::error('PhonePe V2 Status Verification Error: ' . $e->getMessage());
            return null;
        }
    }

    /**
     * Handle PhonePe V2 payment callback
     */
    public function callbackV2(Request $request)
    {
        try {
            ...
            \Log::info('PhonePe V2 Callback Data:', $response);

            $merchantOrderId = $request->query('merchantOrderId') ?? $request->input('merchantOrderId');

            if (!$merchantOrderId) {
                return response()->json(['error' => 'Merchant Order ID not provided in callback.'], 400);
            }

            // Get PhonePe settings for access token
            $phonepay = PaymentGateway::where('type', 'phonepe')->where('status', 1)->first();
            if (!$phonepay) {
                \Log::error('PhonePe payment gateway not configured for callback verification');
                return redirect()->route('payment.failed')->with('error', 'Payment gateway not configured');
            }
            ...
            if (!$accessToken) {
                \Log::error('Failed to get access token for status verification');
                return redirect()->route('payment.failed')->with('error', 'Failed to verify payment status');
            }

            // Verify payment status with PhonePe
            $statusResponse = $this->verifyPaymentStatusV2($merchantOrderId, $accessToken);

            if (!$statusResponse) {
                \Log::error('Failed to get status from PhonePe API');
                return redirect()->route('payment.failed')->with('error', 'Failed to verify payment status');
            }

            \Log::info('PhonePe V2 Status Response:', $statusResponse);

            // Determine payment status based on PhonePe response
            $paymentStatus = 'failed';
            $phonePeTransactionId = null;
            $paymentMessage = 'Payment failed or was cancelled';
            ...
            if (isset($statusResponse['state'])) {
                ...
                if ($paymentState === 'COMPLETED') {
                    $paymentStatus = 'paid';
                    $phonePeTransactionId = $statusResponse['transactionId'] ?? null;
            ...
            if ($paymentStatus === 'paid') {
                \Log::info('Payment successful for transaction ID: ' . $merchantOrderId);

                // Return simple success response
               return redirect('/payment/success?transaction_id=' . $merchantOrderId)->with('success', $paymentMessage);
            } else {
                \Log::info('Payment failed for transaction ID: ' . $merchantOrderId);

                // Redirect to payment failed page with error message
                return redirect('/payment/failed?transaction_id=' . $merchantOrderId)->with('error', $paymentMessage);
            }



        } catch (\Exception $e) {
            \Log::error('PhonePe V2 Callback Error: ' . $e->getMessage());
            return response()->json([
                'success' => false,
