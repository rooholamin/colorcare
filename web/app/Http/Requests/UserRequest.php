    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            if ($this->input('user_type') === 'provider' && request()->is('api/*')) {
                ...
                $requiredDocs = Documents::where('is_required', 1)->where('status', 1)->where('type','!=','shop_document')->pluck('id')->toArray();

                // 1. Check missing required document IDs
                $missingRequired = array_diff($requiredDocs, $submittedIds);
                if (!empty($missingRequired)) {
                    $docNames = Documents::whereIn('id', $missingRequired)->where('type','!=','shop_document')->pluck('name')->toArray();
