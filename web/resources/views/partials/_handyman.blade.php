...
<div class="mb-3">
    <ul class="nav nav-tabs pay-tabs nav--tabs__style2 provider-detail-tab">
        ...
        <li class="nav-item {{ request()->routeIs('handyman.review') ? 'active' : '' }}">
            <a class="nav-link" href="{{ route('handyman.review', $handymandata->id) }}">{{ __('messages.Reviews') }}</a>
        </li>

        <li class="nav-item {{ request()->routeIs('setting.comission_handyman') ? 'active' : '' }}">
            <a class="nav-link" href="{{ route('setting.comission_handyman', $handymandata->id) }}">{{ __('messages.commission') }}</a>
        </li>
