...
<div class="row">
    <div class="form-group">
        <div class="form-control d-flex align-items-center justify-content-between">
            <label for="enable_section_6" class="mb-0">{{ __('messages.discover_our_app') }}</label>
            <div class="custom-control custom-switch custom-switch-text custom-switch-color custom-control-inline">
                <input type="checkbox" class="custom-control-input section_6" name="status" id="section_6"
                    data-type="section_6" {{ !empty($landing_page) && $landing_page->status == 1 ? 'checked' : '' }}>
            ...
            <div class="row">
                <div class="col-sm-4">
                    ...
                    @if ($landing_page && getMediaFileExit($landing_page, 'main_image'))
                        <a class="text-danger remove-file"
                            href="{{ route('remove.file', ['id' => $landing_page->id, 'type' => 'main_image']) }}"
                ...
                <div class="col-sm-8 mt-sm-0 mt-2">
                    <div class="custom-file col-md-12">
                        ...
                        @if ($landing_page && getMediaFileExit($landing_page, 'main_image'))
                            <label
                                class="custom-file-label upload-label">{{ $landing_page->getFirstMedia('main_image')->file_name }}</label>
                        @else
                            <label
            ...
            <div class="row">
                <div class="col-sm-4">
                    ...
                    @if ($landing_page && getMediaFileExit($landing_page, 'google_play'))
                        <a class="text-danger remove-file"
                            href="{{ route('remove.file', ['id' => $landing_page->id, 'type' => 'google_play']) }}"
                            data--submit="confirm_form" data--confirmation='true' data--ajax="true"
                            title='{{ __('messages.remove_file_title', ['name' => __('messages.image')]) }}'
                            data-title='{{ __('messages.remove_file_title', ['name' => __('messages.image')]) }}'
                            data-message='{{ __('messages.remove_file_msg') }}'>
                ...
                <div class="col-sm-8 mt-sm-0 mt-2">
                    <div class="custom-file col-md-12">
                        ...
                        @if ($landing_page && getMediaFileExit($landing_page, 'google_play'))
                            <label
                                class="custom-file-label upload-label">{{ $landing_page->getFirstMedia('google_play')->file_name }}</label>
                        @else
                            <label
                                class="custom-file-label upload-label">{{ __('messages.choose_file', ['file' => __('messages.attachments')]) }}</label>
                        @endif
            ...
            <div class="row">
                <div class="col-sm-4">
                    ...
                    @if ($landing_page && getMediaFileExit($landing_page, 'app_store'))
                        <a class="text-danger remove-file"
                            href="{{ route('remove.file', ['id' => $landing_page->id, 'type' => 'app_store']) }}"
                            data--submit="confirm_form" data--confirmation='true' data--ajax="true"
                            title='{{ __('messages.remove_file_title', ['name' => __('messages.image')]) }}'
                            data-title='{{ __('messages.remove_file_title', ['name' => __('messages.image')]) }}'
                            data-message='{{ __('messages.remove_file_msg') }}'>
                ...
                <div class="col-sm-8 mt-sm-0 mt-2">
                    <div class="custom-file col-md-12">
                        ...
                        @if ($landing_page && getMediaFileExit($landing_page, 'app_store'))
                            <label
                                class="custom-file-label upload-label">{{ $landing_page->getFirstMedia('app_store')->file_name }}</label>
                        @else
                            <label
                                class="custom-file-label upload-label">{{ __('messages.choose_file', ['file' => __('messages.attachments')]) }}</label>
                        @endif
