    ...
    <div class="col-lg-6">
        <div class="form-group">
            ...
            <div class="col-sm-12">
                ...
                <div class="row">
                    <div class="col-sm-4">
                        <img src="{{ getSingleMedia($themesetup, 'logo') }}" width="100" id="logo_preview"
                            alt="logo" class="image logo logo_preview">
                        @if ($themesetup && getMediaFileExit($themesetup, 'logo'))
                            <a class="text-danger remove-file"
                                href="{{ route('remove.file', ['id' => $themesetup->id, 'type' => 'logo']) }}"
                    ...
                    <div class="col-sm-8 mt-sm-0 mt-2">
                        <div class="custom-file col-md-12">
                            {{ html()->file('logo')->class('custom-file-input custom-file-input-sm detail')->id('logo')->attribute('lang', 'en')->attribute('accept', 'image/*')->attribute('onchange', 'preview()') }}
                            @if ($themesetup && getMediaFileExit($themesetup, 'logo'))
                                <label
                                    class="custom-file-label upload-label">{{ $themesetup->getFirstMedia('logo')->file_name }}</label>
                            @else
        ...
        <div class="form-group">
            ...
            <div class="col-sm-12">
                <div class="row">
                    <div class="col-sm-4">
                        <img src="{{ getSingleMedia($themesetup, 'favicon') }}" height="30" id="favicon_preview"
                            alt="favicon" class="image favicon favicon_preview">
                        @if ($themesetup && getMediaFileExit($themesetup, 'favicon'))
                            <a class="text-danger remove-file"
                                href="{{ route('remove.file', ['id' => $themesetup->id, 'type' => 'favicon']) }}"
                                data--submit="confirm_form" data--confirmation='true' data--ajax="true"
                                title='{{ __('messages.remove_file_title', ['name' => __('messages.image')]) }}'
                                data-title='{{ __('messages.remove_file_title', ['name' => __('messages.image')]) }}'
                                data-message='{{ __('messages.remove_file_msg') }}'>
                    ...
                    <div class="col-sm-8 mt-sm-0 mt-2">
                        <div class="custom-file col-md-12">
                            {{ html()->file('favicon')->class('custom-file-input custom-file-input-sm detail')->id('favicon')->attribute('lang', 'en')->attribute('accept', 'image/*')->attribute('onchange', 'preview()') }}
                            @if ($themesetup && getMediaFileExit($themesetup, 'favicon'))
                                <label
                                    class="custom-file-label upload-label">{{ $themesetup->getFirstMedia('favicon')->file_name }}</label>
                            @else
    ...
    <div class="col-lg-6">
        <div class="form-group">
            ...
            <div class="col-sm-12">
                ...
                <div class="row">
                    <div class="col-sm-4">
                        <img src="{{ getSingleMedia($themesetup, 'footer_logo') }}" width="100"
                            id="footer_logo_preview" alt="footer_logo" class="image footer_logo footer_logo_preview">
                        @if ($themesetup && getMediaFileExit($themesetup, 'footer_logo'))
                            <a class="text-danger remove-file"
                                href="{{ route('remove.file', ['id' => $themesetup->id, 'type' => 'footer_logo']) }}"
                                data--submit="confirm_form" data--confirmation='true' data--ajax="true"
                                title='{{ __('messages.remove_file_title', ['name' => __('messages.image')]) }}'
                                data-title='{{ __('messages.remove_file_title', ['name' => __('messages.image')]) }}'
                                data-message='{{ __('messages.remove_file_msg') }}'>
                    ...
                    <div class="col-sm-8 mt-sm-0 mt-2">
                        <div class="custom-file col-md-12">
                            {{ html()->file('footer_logo')->class('custom-file-input custom-file-input-sm detail')->id('footer_logo')->attribute('lang', 'en')->attribute('accept', 'image/*')->attribute('onchange', 'preview()') }}
                            @if ($themesetup && getMediaFileExit($themesetup, 'footer_logo'))
                                <label
                                    class="custom-file-label upload-label">{{ $themesetup->getFirstMedia('footer_logo')->file_name }}</label>
                            @else
                                <label class="custom-file-label upload-label">{{ 'logo.png' }}</label>
                            @endif
                        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-12">
                <div class="row">
                    <div class="col-sm-4">
                        <img src="{{ getSingleMedia($themesetup, 'loader') }}" style="height: 80px; width: auto;"
                            id="loader_preview" alt="loader" class="image loader loader_preview">
                        @if ($themesetup && getMediaFileExit($themesetup, 'loader'))
                            <a class="text-danger remove-file"
                                href="{{ route('remove.file', ['id' => $themesetup->id, 'type' => 'loader']) }}"
                                data--submit="confirm_form" data--confirmation='true' data--ajax="true"
                                title='{{ __('messages.remove_file_title', ['name' => __('messages.image')]) }}'
                                data-title='{{ __('messages.remove_file_title', ['name' => __('messages.image')]) }}'
                                data-message='{{ __('messages.remove_file_msg') }}'>
                    ...
                    <div class="col-sm-8 mt-sm-0 mt-2">
                        <div class="custom-file col-md-12">
                            {{ html()->file('loader')->class('custom-file-input custom-file-input-sm detail')->id('loader')->attribute('lang', 'en')->attribute('accept', 'image/*')->attribute('onchange', 'preview()') }}
                            @if ($themesetup && getMediaFileExit($themesetup, 'loader'))
                                <label
                                    class="custom-file-label upload-label">{{ $themesetup->getFirstMedia('loader')->file_name }}</label>
                            @else
                ...
                <div class="form-group">
                    ...
                    <div class="d-flex align-items-center">
                        <input type="color" name="primary_color" class="form-control"
                            value="{{ isset($themesetup) ? json_decode($themesetup->value)->primary_color ?? '#000000' : '#000000' }}"
...
<script>
    ...
    $(document).ready(function() {
        ...
        // Apply saved colors from database (via form value)
        applySavedColors();

        // If there's a primary color value in the form, apply it immediately
    ...
    function updateThemeColors() {
        ...
        // Get primary color value - prioritize database value (form input), then localStorage
        const primaryColorInput = document.querySelector('input[name="primary_color"]');
        const primaryColor = primaryColorInput && primaryColorInput.value ? primaryColorInput.value : localStorage
            .getItem('primaryColor');
    ...
    function applySavedColors() {
        ...
        // First try to get color from database (form input value)
        const primaryColorInput = document.querySelector('input[name="primary_color"]');
        const savedPrimaryColor = primaryColorInput && primaryColorInput.value ? primaryColorInput.value : localStorage
            .getItem('primaryColor');
        ...
        if (savedPrimaryColor) {
            ...
            localStorage.setItem('primaryColor', savedPrimaryColor);
        }
    }
    ...
    function saveThemeColors(event) {
        ...
        // The form submission will save to database automatically
        // Note: Database save ensures color persists across browsers
    }
